...
class MultiAltEndpointsConnection extends internal_1.Listenable {
    ...
    onConnecting(connection) {
        console.debug(`onConnecting event triggered by id: ${connection.id()}, endpoint: ${connection.endpoint()}`);
        this._eventHandler.onConnecting(this, connection);
        this.notify(Event.ON_CONNECTING, this, connection);
    }
    onConnected(connection) {
        console.debug(`onConnected event triggered by id: ${connection.id()}, endpoint: ${connection.endpoint()}`);
        this._condition.notify();
        this._eventHandler.onConnected(this, connection);
        this.notify(Event.ON_CONNECTED, this, connection);
    }
    onDisconnecting(connection) {
        console.debug(`onDisconnecting event triggered by id: ${connection.id()}, endpoint: ${connection.endpoint()}`);
        this._eventHandler.onDisconnecting(this, connection);
        this.notify(Event.ON_DISCONNECTING, this, connection);
    }
    onDisconnected(connection) {
        if (!process.env.RUN_IN_JEST) {
            console.debug(`onDisconnected event triggered by id: ${connection.id()}, endpoint: ${connection.endpoint()}`);
        }
        this._eventHandler.onDisconnected(this, connection);
        this.notify(Event.ON_DISCONNECTED, this, connection);
    }
    onCorrupted(connection) {
        console.debug(`onCorrupted event triggered by id: ${connection.id()}, endpoint: ${connection.endpoint()}`);
        this._reconnect();
