...
exports.makeConnectionOptions = makeConnectionOptions;
exports.makeConnectionPoolOptions = makeConnectionPoolOptions;
...
function makeConnectionOptions(options) {
...
class MultiAltEndpointsConnection extends internal_1.Listenable {
    ...
    onConnecting(connection, ...rest) {
        console.debug(`<${this.name()}>Connecting: endpoint: ${connection.endpoint()}`);
    ...
    onConnected(connection, ...rest) {
        console.debug(`<${this.name()}>Connection connected: endpoint: ${connection.endpoint()}`);
        this._readyState = ReadyState.OPEN;
        this._openCondition.notify();
    ...
    onDisconnecting(connection, ...rest) {
        console.debug(`<${this.name()}>Disconnecting: endpoint: ${connection.endpoint()}`);
    ...
    onDisconnected(connection, ...rest) {
        console.debug(`<${this.name()}>Connection disconnected: endpoint: ${connection.endpoint()}`);
        this._readyState = ReadyState.CLOSED;
        if (!this._shouldRun) {
            this._closedCondition.notify();
        }
    ...
    onCorrupted(connection, ...rest) {
        console.debug(`<${this.name()}>Connection corrupted: endpoint: ${connection.endpoint()}`);
    ...
    onBecameUnhealthy(connection, ...rest) {
        console.debug(`<${this.name()}>Connection became unhealthy: endpoint: ${connection.endpoint()}`);
    ...
    onBecameHealthy(connection, ...rest) {
        console.debug(`<${this.name()}>Connection became healthy: endpoint: ${connection.endpoint()}`);
    ...
    onBecameActive(connection, ...rest) {
        console.debug(`<${this.name()}>Connection became active: endpoint: ${connection.endpoint()}`);
    ...
    onBecameIdle(connection, ...rest) {
        console.debug(`<${this.name()}>Connection became idle: endpoint: ${connection.endpoint()}`);
...
function makeConnectionPoolOptions(options) {
    if (typeof options === "undefined") {
        options = {};
    }
    return {
...
class ConnectionPool extends internal_1.Listenable {
    ...
    waitAllOpen(timeout) {
        const promises = this._allConnections.map((connection) => connection.waitOpen(timeout));
    ...
    onConnecting(connection, ...rest) {
        console.debug(`<${this.name()}>Connecting: name: ${connection.name()}, endpoint: ${connection.endpoint()}`);
        tryWith(connection, () => this._eventHandler.onConnecting?.(connection, ...rest));
        this.notify(Event.ON_CONNECTING, connection, ...rest);
    }
    onConnected(connection, ...rest) {
        console.debug(`<${this.name()}>Connection connected: name: ${connection.name()}, endpoint: ${connection.endpoint()}`);
        tryWith(connection, () => this._eventHandler.onConnected?.(connection, ...rest));
        this.notify(Event.ON_CONNECTED, connection, ...rest);
    }
    onDisconnecting(connection, ...rest) {
        console.debug(`<${this.name()}>Disconnecting: name: ${connection.name()}, endpoint: ${connection.endpoint()}`);
        tryWith(connection, () => this._eventHandler.onDisconnecting?.(connection, ...rest));
        this.notify(Event.ON_DISCONNECTING, connection, ...rest);
    }
    onDisconnected(connection, ...rest) {
        console.debug(`<${this.name()}>Connection disconnected: name: ${connection.name()}, endpoint: ${connection.endpoint()}`);
    ...
    onCorrupted(connection, ...rest) {
        console.debug(`<${this.name()}>Connection corrupted: name: ${connection.name()}, endpoint: ${connection.endpoint()}`);
        this._dropConnection(connection);
        tryWith(connection, () => this._eventHandler.onCorrupted?.(connection, ...rest));
        this.notify(Event.ON_CORRUPTED, connection, ...rest);
    }
    onBecameUnhealthy(connection, ...rest) {
        console.debug(`<${this.name()}>Connection became unhealthy: name: ${connection.name()}, endpoint: ${connection.endpoint()}`);
    ...
    onBecameHealthy(connection, ...rest) {
        console.debug(`<${this.name()}>Connection became healthy: name: ${connection.name()}, endpoint: ${connection.endpoint()}`);
        this._updateConnectionHealth(connection);
        tryWith(connection, () => this._eventHandler.onBecameHealthy?.(connection, ...rest));
    }
    onBecameIdle(connection, ...rest) {
        console.debug(`<${this.name()}>Connection became idle: name: ${connection.name()}, endpoint: ${connection.endpoint()}`);
        this._dropConnection(connection);
        tryWith(connection, () => this._eventHandler.onBecameIdle?.(connection, ...rest));
        this.notify(Event.ON_BECAME_IDLE, connection, ...rest);
    }
    onBecameActive(connection, ...rest) {
        console.debug(`<${this.name()}>Connection became active: name: ${connection.name()}, endpoint: ${connection.endpoint()}`);
