...
class ConnectionPool extends internal_1.Listenable {
    ...
    onDisconnected(connection, ...rest) {
        if (connection.isClosed()) {
            console.debug(`<${this.name()}>Connection was closed, dropping it: name: ${connection.name()},endpoint: ${connection.endpoint()}`);
    ...
    onCorrupted(connection, ...rest) {
        console.debug(`<${this.name()}>Connection corrupted, dropping it: name: ${connection.name()}, endpoint: ${connection.endpoint()}`);
        this._dropConnection(connection);
    ...
    onBecameIdle(connection, ...rest) {
        console.debug(`<${this.name()}>Connection became idle, dropping it: name: ${connection.name()}, endpoint: ${connection.endpoint()}`);
        this._dropConnection(connection);
