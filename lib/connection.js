...
class Connection extends internal_1.Listenable {
    ...
    _onError(e) {
        console.error(`Connection corrupted: endpoint: ${this._endpoint}, error: ${e.message}`);
...
class MultiAltEndpointsConnection extends internal_1.Listenable {
    constructor(pickEndpoint, options) {
        super();
    ...
    request(msg, timeout) {
        return abortable_promise_1.AbortablePromise.from(this._condition.wait(timeout)).then(() => {
